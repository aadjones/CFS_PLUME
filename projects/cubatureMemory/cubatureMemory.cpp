...
int main(int argc, char* argv[])
{
  ...
  int prediffuseSize = prediffuseU.cols();
  int preadvectSize = 0; // set this later once preadvect is in core
  ...
  if (file == NULL)
  {
    ...
    // get the dimensions
    // ADJ: we assume the dimensions of preadvect and prediffuse are the same
    FILE* file;
    ...
    // postadvect (aka prediffuse) loop
    for (int x = 0; x < reducedSnapshots; x++)
    {
      ...
      VectorXd approx = prediffuseU * (prediffuseU.transpose() * prediffuse);
      VectorXd diff = prediffuse - approx;
      cout << " Postadvection projection error: " << diff.norm() << endl;
    }

    subspaceFluid.wipePrediffuse();

    subspaceFluid.loadPreadvect();
    const MatrixXd& preadvectU = subspaceFluid.preadvectU();
    preadvectSize = preadvectU.cols();
    ...
    // don't wipe preadvect since we use it in the next routine
  ...
  cout << "================================================================" << endl;
  cout << " Generating greedy refined cubature for input basis rank " << preadvectSize << endl;
  cout << "   and output basis rank " << prediffuseSize << endl;
  cout << "================================================================" << endl;


  string prediffusePath = reducedPath + string("U.prediffuse.matrix");
  generator.preprocess_prediffuse(prediffusePath);

  // reversed since it is the transpose
  generator.allocate_postU_T(generator.prediffuseU_cols(), 3);

  // generator.generateImportanceSampledCubature();
  generator.generateImportanceSampledCubatureMemory();
